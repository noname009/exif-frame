...
const useStore = create<Store>((set) => ({
  ...
  selectedThemeName: themes.find((theme) => theme.name === localStorage.getItem('selectedThemeName'))?.name
    // 첫 접속 시 기본 = 'Histogram' (첫 신규 테마). 클래식 토글 닫힘 상태 유지.
    || themes.find((t) => t.name === 'Histogram')?.name
    || themes[0].name,
